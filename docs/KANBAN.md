...
| SM-14 | Cached data can outlive the session that fetched it on a shared device | P0 | none needed | **NEW 2026-09-01. Mechanism, repro steps and fix are held in the private tracker while this is unfixed — see the review's note at the bottom of this file.** In short: on a device more than one person signs into, data cached for one session can outlive it. Realistic on a shared sideline tablet. **Why here:** it is the only finding in the review that exposes one user's data to another, and SM-1 widens its blast radius by adding game data to the cache. |
