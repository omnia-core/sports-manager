...
|---|---|---|---|---|
| SM-9 | The coach is force-logged-out mid-game | P0 | none needed | **NEW 2026-09-01, found by the review, not on the original board.** `AccessTokenTTL = 60 * time.Minute` (`backend/internal/auth/jwt.go:15`) and the frontend never calls `/api/auth/refresh` — `frontend/src/api/auth.ts` has only register/login/logout/me. `client.ts:22` fires `onUnauthorized` on any 401, clearing the session and redirecting to `/login`. A game with warm-up runs 90-120 minutes, so this fires around the third quarter, every game. **Fix:** on 401, attempt `POST /api/auth/refresh` once as a single-flight shared promise, replay the original request, and fall through to `onUnauthorized` only if the refresh itself fails. **Why first:** without it SM-1 builds a durable queue for a session guaranteed to be terminated mid-game — and worse, after SM-1 the coach keeps tapping into an optimistic UI while every write 401s. |
| SM-14 | Cached data can outlive the session that fetched it on a shared device | P0 | none needed | **NEW 2026-09-01. Details held back from this public repo while unfixed.** The service worker retains authenticated API responses across a logout, so a second person signing in on the same device can be served the first person's data when the network is slow or absent. Realistic on a shared sideline tablet. **Fix:** clear the caches as part of sign-out, before the auth state is cleared, and scope any queued writes to the user who created them. Full mechanism and repro steps are in the private tracker. **Why here:** it is the only finding in the review that exposes one user's data to another, and SM-1 widens its blast radius by adding game data to the cache. |
